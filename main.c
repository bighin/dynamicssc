...
{
	fprintf(out,"# <Time> <NormQP L=0> <NormPhonons L=0> ... <NormQP L=Lmax> <NormPhonons L=Lmax> <TotalNorm> <Re(AlignmentCosine)> <Im(AlignmentCosine)> <Re(Cos^2)> <Im(Cos^2)> <Re(S(t))> <Im(S(t))> <LaserIntensity> <BathIntensity> <MolecularRotationalEnergy> <BosonsRotationalEnergy> <TotalRotationalEnergy> <Hamiltonian>\n");
}

void print_header_summary(FILE *out)
{
	fprintf(out,"# <Time> <LaserIntensity> <BathIntensity> <Norm> <NormQP> <NormPhonons> <Re(AlignmentCosine)> <Im(AlignmentCosine)> <Re(S(t))> <Im(S(t))> <Completion%%> <LocalNormErr> <TimeDE> <TimeCos> <Torque> <RotationalEnergy> <MolecularRotationalEnergy> <BosonsRotationalEnergy> <TotalRotationalEnergy> <Hamiltonian>\n");
}
...
int do_single(struct configuration_t *config)
{
	...
	for(int c=0;c<=timedivs;c++)
	{
		...
		double trq;
		double molecular_rotational_e,bosons_rotational_e,total_rotational_e,total_e;
		...
		*/
		...
		fprintf(norms,"%f %f %f %f %f %f %f %f %f %f %f %f %f\n",total_norm(psi),creal(ac),cimag(ac),creal(cs),cimag(cs),creal(S),cimag(S),get_laser_intensity(config->fluence,config->duration,ti,config),bath_intensity,molecular_rotational_e,bosons_rotational_e,total_rotational_e,total_e);
		fflush(norms);
		...
		*/
		...
		printf("%f %f %f %f %f %f %f %f\n",previousnorm,elapsed_time1,elapsed_time2,trq,molecular_rotational_e,bosons_rotational_e,total_rotational_e,total_e);
		fflush(stdout);
	}
